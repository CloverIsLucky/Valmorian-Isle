...
/obj/item/organ/wings/harpy/Insert(mob/living/carbon/human/M, special = FALSE, drop_if_replaced = TRUE)
	var/already_inserted = (owner == M) //job equip re-Inserts should_regenerate organs; don't stack talons/skin
	. = ..()
	if(isharpy(M))
		if(M.mind)
		...
		if(!already_inserted)
			...
			if(!istype(M.skin_armor, /obj/item/clothing/suit/roguetown/armor/regenerating/skin/harpy_skin))
				M.skin_armor = new /obj/item/clothing/suit/roguetown/armor/regenerating/skin/harpy_skin
		add_verb(M, list(/mob/living/carbon/human/proc/harpy_fly_up, /mob/living/carbon/human/proc/harpy_fly_down))
	else if(M.mind)
...
/obj/item/organ/wings/harpy/Remove(mob/living/carbon/human/M, special = FALSE, drop_if_replaced = TRUE)
	...
	M.remove_status_effect(/datum/status_effect/debuff/harpy_flight)
	remove_verb(M, list(/mob/living/carbon/human/proc/harpy_fly_up, /mob/living/carbon/human/proc/harpy_fly_down))
	if(istype(M.skin_armor, /obj/item/clothing/suit/roguetown/armor/regenerating/skin/harpy_skin)) //the leg armor belongs to the wings
		qdel(M.skin_armor)
		M.skin_armor = null
	QDEL_LIST(nullspace_items)
