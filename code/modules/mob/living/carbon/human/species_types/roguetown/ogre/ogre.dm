...
/datum/species/ogre
	...
	origin = "Gronn"
	is_subrace = TRUE
	base_name = "Humen"	//VALMORIAN: ES kept an Ogroid family with Half-Orc as base; VI folded Half Orc under Humen, so the Ogre follows - race menu Humen -> Subrace -> Ogre, as EMS players picked it via Half-Orc -> Ogre
	sub_name = "Ogre"	//Valmorian: the subrace picker keys and filters on sub_name; without it the species is unpickable
	desc = "<b>Ogres</b><br>\
	...
	race_bonus = list(STAT_STRENGTH = 2, STAT_CONSTITUTION = 2, STAT_INTELLIGENCE = -3, STAT_SPEED = -1) //VALMORIAN: no endurance stat here; ES gave +1 END
	...
	job_whitelist = list(/datum/job/roguetown/ogre)	//ogres take ogre roles only, as on Emerald Summit
	fixed_body_size = TRUE	//size is baked into the 64px sprites; a scale transform on top just distorts
	preview_pixel_gap = 8
	languages = list(
