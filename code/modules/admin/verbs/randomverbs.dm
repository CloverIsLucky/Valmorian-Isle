...
/client/proc/admin_spawn_cake(mob/living/M in GLOB.mob_list)
	...
	var/cake_msg = span_adminnotice("[key_name_admin(usr)] gave a cake slice to [ADMIN_LOOKUPFLW(M)].")
	message_admins(cake_msg)
	...
	SSblackbox.record_feedback("tally", "admin_verb", 1, "Give Cake Slice")
